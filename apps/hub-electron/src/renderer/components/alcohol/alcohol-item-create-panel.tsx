...
import { messageOf, rupeesToPaise, type NoticeSetter } from "../../lib/format.js";
...
function AlcoholItemCreatePanel({
  bootstrap,
  catalog,
  invalidate,
  setNotice
  ...
  const [unitId, setUnitId] = useState(
    bootstrap.productionUnits.find((unit) => unit.name.toLowerCase().includes("bar"))?.id ?? "",
  );
      ...
      return hubApi.createAlcoholItem({
        ...
        recipeIngredients:
          type === "prepared_product"
            ? recipe
                .filter((row) => row.liquorMenuItemId && Number(row.mlPerUnit) > 0)
                .map((row) => ({
  ...
  const canSubmit =
    name.trim() &&
    (type === "plain_liquor"
      ? [shotPrice, smallPrice, largePrice].some((value) => rupeesToPaise(value) > 0)
        ...
        {type === "plain_liquor" ? (
          <>
            <div className="three-col">
              <label>
                Large ml
                <input value={largeMl} onChange={(event) => setLargeMl(event.target.value)} inputMode="numeric" />
              </label>
              <label>
                Small ml
                <input value={smallMl} onChange={(event) => setSmallMl(event.target.value)} inputMode="numeric" />
              </label>
              <label>
                Open large ml
                <input value={openMl} onChange={(event) => setOpenMl(event.target.value)} inputMode="numeric" />
              </label>
            </div>
            <div className="three-col">
              <label>
                Large bottles
                <input value={sealedLarge} onChange={(event) => setSealedLarge(event.target.value)} inputMode="numeric" />
              </label>
              <label>
                Small bottles
                <input value={sealedSmall} onChange={(event) => setSealedSmall(event.target.value)} inputMode="numeric" />
              </label>
              <span />
            </div>
            <div className="three-col">
              <label>
                30 ml price
                <input value={shotPrice} onChange={(event) => setShotPrice(event.target.value)} inputMode="decimal" placeholder="120" />
              </label>
              <label>
                Small bottle price
                <input value={smallPrice} onChange={(event) => setSmallPrice(event.target.value)} inputMode="decimal" placeholder="480" />
              </label>
              <label>
                Large bottle price
                <input value={largePrice} onChange={(event) => setLargePrice(event.target.value)} inputMode="decimal" placeholder="1800" />
              </label>
            </div>
          </>
        ) : (
          <>
            <label>
              Price
              <input value={productPrice} onChange={(event) => setProductPrice(event.target.value)} inputMode="decimal" placeholder="350" />
            </label>
            <div className="recipe-list">
                ...
                <div className="recipe-row" key={index}>
                    ...
                    onChange={(event) =>
                      setRecipe((current) =>
                        current.map((entry, entryIndex) =>
                          entryIndex === index ? { ...entry, liquorMenuItemId: event.target.value } : entry,
                        ),
                      )
                    }
                  >
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setRecipe((current) =>
                        current.map((entry, entryIndex) =>
                          entryIndex === index ? { ...entry, mlPerUnit: event.target.value } : entry,
                        ),
                      )
                    }
                    inputMode="numeric"
                    placeholder="ml"
                  />
                  <button type="button" onClick={() => setRecipe((current) => current.filter((_, entryIndex) => entryIndex !== index))}>
                    <X size={14} />
                  </button>
                </div>
              ))}
              <button
                ...
                onClick={() => setRecipe((current) => [...current, { liquorMenuItemId: plainLiquors[0]?.id ?? "", mlPerUnit: "30" }])}
              >
                <Plus size={14} />
                Add liquor
              </button>
